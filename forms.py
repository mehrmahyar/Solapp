...
# Placeholder - Health Questionnaire needs dynamic fields based on defined questions
# For now, a simple textarea
class HealthQuestionnaireForm(FlaskForm):
    responses_text = TextAreaField('Health Information (Use format: Question=Answer)', validators=[DataRequired()], render_kw={"rows": 10})
...
class TrainingSessionForm(FlaskForm):
    ...
    # Similar to program form, logs are complex. Use TextArea for JSON for now.
    # TODO: Implement dynamic log entry based on selected program in future
    logs_json_text = TextAreaField('Detailed Logs (JSON format - see model)', validators=[Optional()], render_kw={"rows": 15})
